...
function reportBootMetrics() {
  ...
  const htmlSplashVisible = performance.getEntriesByName(bootMarks.htmlSplashVisible, "mark")[0];
  ...
  const summary = {
    reactMounted: Math.round(
      (performance.getEntriesByName(bootMarks.reactMounted, "mark")[0]?.startTime ??
        htmlSplashVisible.startTime) - htmlSplashVisible.startTime,
    ),
    authResolved: Math.round(
      (performance.getEntriesByName(bootMarks.authResolved, "mark")[0]?.startTime ??
        htmlSplashVisible.startTime) - htmlSplashVisible.startTime,
    ),
    initialReady: Math.round(
      (performance.getEntriesByName(bootMarks.initialReady, "mark")[0]?.startTime ??
        htmlSplashVisible.startTime) - htmlSplashVisible.startTime,
    ),
  };
...
export function BootFlowProvider({ children }: PropsWithChildren) {
  ...
  const value = useMemo<BootContextValue>(
    ...
    [isInitialBootPending, markReactMounted, markAuthResolved, markInitialScreenReady],
  );

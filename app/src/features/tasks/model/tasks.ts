...
export const TaskTypeConst = TaskTypeValues;
...
export function canSubmitTaskForm(form: TaskFormState) {
  return (
    ...
    (!isWeeklyTaskType(form.type) ||
      getWeeklyRequiredCompletionsError(form.requiredCompletionsPerWeek) == null)
  );
}

export function buildCreateTaskRequest(form: TaskFormState): CreateTaskRequest | { error: string } {
  let requiredCompletionsPerWeek: number | undefined;
  if (isWeeklyTaskType(form.type)) {
    const error = getWeeklyRequiredCompletionsError(form.requiredCompletionsPerWeek);
    if (error != null) {
      return { error };
    }

...
describe("TasksPage", () => {
  ...
  it("shows a created task at the top of its type immediately", async () => {
    ...
    await waitFor(() => {
      const dailySection = screen.getByRole("heading", { name: "毎日" }).closest("section");
  ...
  it("resets task form fields after creating task", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByRole("dialog", { name: "タスクを追加" })).not.toBeInTheDocument();
    });
  ...
  it("reorders daily tasks from drag-and-drop", async () => {
    ...
    renderPage();
    expect(
      await screen.findByText("タスク設定はチーム共通です。並び替えは管理画面でのみ変更できます。"),
    ).toBeInTheDocument();
  ...
  it("cancels editing and restores display state", async () => {
    ...
    expect(within(displayCard).queryByLabelText("タイトル")).not.toBeInTheDocument();
  });

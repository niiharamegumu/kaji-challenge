...
const mockFullCalendar = vi.fn((props: Record<string, unknown>): ReactElement => (
...
vi.mock("../hooks/useReminders", () => ({
  ...
  useReminderCalendarQuery: (monthKey: string) => mockUseReminderCalendarQuery(monthKey),
...
vi.mock("@fullcalendar/react", async () => {
  ...
  return {
    default: React.forwardRef((props: Record<string, unknown>, _ref) => mockFullCalendar(props)),
  };
});
...
describe("ReminderCalendarPage", () => {
  ...
  it("shows mobile-specific description and does not pass events to FullCalendar on mobile", () => {
    ...
    expect(screen.getByRole("dialog", { name: "4月2日(木) のリマインダー" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "追加" })).toBeInTheDocument();
    expect(
      within(screen.getByRole("dialog", { name: "4月2日(木) のリマインダー" })).queryByRole(
        "button",
        { name: "追加" },
      ),
  ...
  it("keeps the mobile agenda sheet open after creating a reminder", async () => {
    ...
    expect(
      screen.getAllByRole("dialog", { name: "4月2日(木) のリマインダー" }).length,
    ).toBeGreaterThan(0);
  });
  ...
  it("opens the create sheet from the footer quick action on desktop", () => {
    ...
    expect(screen.getByRole("dialog", { name: "リマインダーを追加" })).toBeInTheDocument();
  });
  ...
  it("shows desktop drag-and-drop description and passes events to FullCalendar on desktop", () => {
    ...
    expect((latestCalendarProps()?.events as unknown[] | undefined)?.length).toBe(1);
  ...
  it.each([390, 1280])(
    "edits a recurring reminder and retains validation at width %i",
    async (width) => {
      setViewport(width);
      ...
      // The mobile page also renders a hidden desktop agenda; use the visible sheet.
      const agenda =
        width < 768
          ? within(screen.getByRole("dialog", { name: "4月2日(木) のリマインダー" }))
          : screen;
      fireEvent.click(agenda.getByRole("button", { name: "sample を編集" }));
      const edit = within(screen.getByRole("dialog", { name: "リマインダーを編集" }));
      expect(edit.getByLabelText("タイトル")).toHaveValue("sample");
      expect(edit.getByLabelText("くり返し")).toHaveValue("weekly");
      expect(edit.getByLabelText("開始日")).toHaveValue("2026-04-02");
      fireEvent.change(edit.getByLabelText("タイトル"), { target: { value: " " } });
      expect(edit.getByRole("button", { name: "更新する" })).toBeDisabled();
      fireEvent.change(edit.getByLabelText("タイトル"), { target: { value: "updated reminder" } });
      fireEvent.change(edit.getByLabelText("メモ"), { target: { value: "updated notes" } });
      await act(async () => {
        fireEvent.click(edit.getByRole("button", { name: "更新する" }));
      });
      expect(updateReminder).toHaveBeenCalledExactlyOnceWith({
        reminderId: "rem-1",
        payload: {
      ...
      expect(screen.queryByRole("dialog", { name: "リマインダーを編集" })).not.toBeInTheDocument();
    },
  );

  it.each([390, 1280])(
    "keeps deletion confirmation above the edit sheet at width %i",
    async (width) => {
      setViewport(width);
      ...
      fireEvent.click(agenda.getByRole("button", { name: "sample を編集" }));
      const edit = within(screen.getByRole("dialog", { name: "リマインダーを編集" }));
      fireEvent.click(edit.getByRole("button", { name: /^削除$/ }));
      const confirm = () =>
        within(screen.getByRole("dialog", { name: "リマインダーを削除しますか" }));
      expect(removeReminder).not.toHaveBeenCalled();
      fireEvent.click(confirm().getByRole("button", { name: "キャンセル" }));
      expect(edit.getByLabelText("タイトル")).toHaveValue("sample");
      fireEvent.click(edit.getByRole("button", { name: /^削除$/ }));
      await act(async () => {
        fireEvent.click(confirm().getByRole("button", { name: "削除する" }));
      });
      expect(removeReminder).toHaveBeenCalledExactlyOnceWith("rem-1");
      expect(screen.queryByRole("dialog", { name: "リマインダーを編集" })).not.toBeInTheDocument();
    },
  );
});

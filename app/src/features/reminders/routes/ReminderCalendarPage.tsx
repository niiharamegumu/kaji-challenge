...
function normalizeVisibleDateKey(dateKey: string, todayKey: string) {
  return monthKeyFromDateKey(dateKey) < monthKeyFromDateKey(todayKey) ? todayKey : dateKey;
}
          ...
          <select
            ...
            onChange={(event) =>
              onChange({
                ...
                endDate: event.target.value === ReminderKindConst.one_time ? "" : form.endDate,
              })
            }
          >
...
function ReminderAgendaList({
  ...
  if (selectedOccurrences.length === 0) {
    return (
      <p className={className ?? "text-sm text-stone-500"}>この日のリマインダーはありません。</p>
    );
  }
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <div className="font-medium text-stone-900">{occurrence.title}</div>
...
function MobileAgendaSheet({
  ...
  return createPortal(
    <>
      ...
      <dialog
        open
        className="fixed bottom-2 left-1/2 z-[70] w-[min(42rem,calc(100%-1rem))] -translate-x-1/2 overflow-hidden rounded-[1.75rem] border border-stone-200 bg-white p-0 shadow-2xl"
        aria-modal="true"
        aria-label={`${dateLabel} のリマインダー`}
      >
        <div className="flex max-h-[calc(100dvh-1rem-env(safe-area-inset-top))] flex-col px-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <CalendarDays size={18} aria-hidden="true" className="text-stone-600" />
              <h3 className="text-base font-semibold text-stone-900">{dateLabel}</h3>
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
          ...
          <div className="mt-3 min-h-0 flex-1 overflow-y-auto px-px">
            <ReminderAgendaList
              selectedOccurrences={selectedOccurrences}
              reminderMap={reminderMap}
              className={selectedOccurrences.length === 0 ? "text-sm text-stone-500" : "space-y-2"}
              onEdit={onEdit}
            />
          </div>
        </div>
...
export function ReminderCalendarPage() {
  ...
  const initialSelectedDate = normalizeVisibleDateKey(normalizeDateKey(initialDateParam), todayKey);
  ...
  const [visibleMonth, setVisibleMonth] = useState(monthKeyFromDateKey(initialSelectedDate));
  ...
  const calendarDays = useMemo(() => calendarQuery.data ?? [], [calendarQuery.data]);
  ...
  const renderDayCellContent = (arg: DayCellContentArg) => {
    ...
    const dayNumber = String(arg.date.getUTCDate());
    const mobileDots = mobileDotIds.slice(0, Math.min(occurrenceCount, mobileMarkerLimit));
          ...
          <div className="flex w-full items-start justify-between">
            ...
            <button
              ...
              className={`reminder-day-number inline-flex min-h-7 min-w-7 cursor-pointer items-center justify-center rounded-full px-1.5 text-xs font-medium transition-colors ${
                isSelected ? "bg-amber-100 text-stone-900" : "text-stone-700 hover:bg-stone-100"
      ...
      >
        ...
        <div className="mt-4 flex items-center justify-between gap-3 px-2 md:px-0">
          <div className="flex min-w-0 items-center gap-2">
            <button
              type="button"
              className={canShowPreviousMonth ? calendarButtonClass : disabledCalendarButtonClass}
        ...
        >
          {/* All events are date-only. UTC preserves their keys; today is determined in JST. */}
          <FullCalendar
            ref={calendarRef}
            locale={jaLocale}
            ...
            editable
            dayMaxEventRows={3}
            height={isMobile && mobileCalendarHeight != null ? mobileCalendarHeight : "auto"}
            longPressDelay={250}
            eventDurationEditable={false}
            dateClick={handleDateClick}
            eventClick={handleEventClick}
            eventDrop={handleEventDrop}
            eventAllow={(dropInfo) => dropInfo.startStr.slice(0, 10) >= todayKey}
            ...
            datesSet={(arg) => {
              const monthKey = formatDateKey(arg.view.currentStart).slice(0, 7);
        ...
        <section className="mt-4 hidden rounded-2xl border border-stone-200 bg-stone-50 px-3 py-3 md:block">
          ...
          <ReminderAgendaList
            ...
            reminderMap={reminderMap}
            className={
              selectedOccurrences.length === 0 ? "mt-3 text-sm text-stone-500" : "mt-3 space-y-2"
            }
            onEdit={openEditSheet}
          />
        </section>
        <p className="mt-4 text-xs text-stone-500">{calendarDescription(isMobile)}</p>
      </article>
        ...
        footerStart={
          editingReminderId != null ? (
            <button
              ...
              onClick={() => setPendingDeleteId(editingReminderId)}
            >

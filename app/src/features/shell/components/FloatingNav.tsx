...
import { useLocation, useNavigate } from "../../../shared/router/navigation";
...
export function FloatingNav({
  ...
  const indicatorIndex = dragIndex ?? hoverIndex ?? focusIndex ?? settledIndex ?? activeIndex;
  const hasInteractiveHoverOrFocus = hoverIndex != null || focusIndex != null;
  const isInteractiveGlass = dragIndex != null || (!menuOpen && hasInteractiveHoverOrFocus);
  const isMoreSelected = menuOpen || activeIndex == null;
  const indicatorX = isInteractiveGlass ? indicator.x - 3 : indicator.x;
  const indicatorWidth = isInteractiveGlass ? indicator.width + 6 : indicator.width;
  ...
  useEffect(() => {
    const updateIndicator = () => {
      ...
      const itemRect = itemRefs.current[indicatorIndex]?.getBoundingClientRect();
  ...
  const handlePrimaryPointerLeave = (event: ReactPointerEvent<HTMLButtonElement>) => {
    if (event.pointerType !== "mouse") {
      return;
    }
  ...
  const handlePrimaryFocus = (index: number, event: ReactFocusEvent<HTMLButtonElement>) => {
    ...
    for (const [index] of primaryItems.entries()) {
      ...
      const centerX =
        rect != null
          ...
          : navRect.left + (navRect.width / primaryItems.length) * (index + 0.5);
  ...
  const handlePrimaryPointerDown = (index: number, event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    <>
      ...
      >
        <div
          ...
          style={{
            background: "linear-gradient(180deg, rgba(255,255,255,0.5), rgba(246,241,232,0.34))",
        ...
        <div className="flex items-end gap-2.5">
          <div
            ...
            style={{
              background: "linear-gradient(180deg, rgba(244,242,238,0.52), rgba(225,221,214,0.22))",
              backdropFilter: "blur(30px) saturate(220%) brightness(0.94) contrast(1.08)",
              WebkitBackdropFilter: "blur(30px) saturate(220%) brightness(0.94) contrast(1.08)",
              touchAction: "pan-y",
            }}
            ...
            {primaryItems.map((item, index) => {
              ...
              return (
                <button
                  ...
                  onPointerDown={(event) => handlePrimaryPointerDown(index, event)}
                  onPointerEnter={(event) => handlePrimaryPointerEnter(index, event)}
            ...
            {isMoreSelected ? (
              <span
                ...
                style={{
                  ...
                  WebkitBackdropFilter: "blur(14px) saturate(160%) brightness(1.02)",
                  boxShadow:

...
export function StatusToast({ message, onDismiss, actionLabel, onAction }: Props) {
  ...
  return (
    <output
      className="fixed z-50 w-[min(80vw,22rem)] top-2 right-2 bottom-auto left-auto w-[22rem] translate-x-0"
      aria-live="polite"
    ...
    >
      <div className="flex items-center gap-2 rounded-lg border border-stone-200 bg-white/95 px-2.5 py-1.5 text-stone-800 shadow-md backdrop-blur">
        ...
        <p className="min-w-0 flex-1 text-xs leading-5 break-words">{message}</p>
        {hasAction ? (
          <button
            type="button"

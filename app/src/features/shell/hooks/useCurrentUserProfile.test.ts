...
import type { MeResponse, TeamMember, User } from "../../../lib/api/operations";
...
describe("useCurrentUserProfile", () => {
  ...
  it("falls back to me displayName and me color when member data is missing", () => {
    ...
    const { result } = renderHook(() => useCurrentUserProfile(meData, undefined));
  ...
  it("returns safe defaults when me data is undefined", () => {
    const { result } = renderHook(() => useCurrentUserProfile(undefined, undefined));

...
describe("MonthCloseBanner", () => {
  ...
  it("shows the oldest pending month and remaining count", () => {
    ...
    expect(screen.getByText("2026年1月の月次締めが必要です")).toBeInTheDocument();
    expect(screen.getByText(/ほか2か月/)).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "この月を締める" })).toHaveAttribute(
      "href",
      "/summary?month=2026-01&close=1",
    );
  });
  ...
  it("keeps the page usable and retries after a fetch error", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "月次締め状態を再取得" }));

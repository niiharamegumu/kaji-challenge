...
export function WeeklyTasksPanel({
  ...
  const normalizedElapsedDaysInWeek = Math.min(7, Math.max(0, elapsedDaysInWeek));
  ...
  return (
    <article className={HOME_PANEL_CLASS_NAME}>
      ...
      {items.length === 0 ? (
        <p className="mt-2 px-2 text-sm text-stone-500 md:px-0">週間タスクはありません。</p>
      ) : (
        <ul className="mt-2 space-y-2">
          {items.map((item) => {
            ...
            const isDone = item.weekCompletedCount >= item.requiredCompletionsPerWeek;
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="mt-1 flex items-center justify-between gap-2">
                        <div className="flex min-w-0 flex-wrap items-center gap-1.5 text-xs text-stone-600">
                          ...
                          <span>
                            進捗 {item.weekCompletedCount}/{item.requiredCompletionsPerWeek}
                          </span>
                        </div>
                        <CompletionSlots
                          compact
            ...
            return (
              <li key={item.task.id} className="relative overflow-visible">
                <div
                  className={`relative overflow-hidden rounded-xl p-2.5 ring-1 ${isDone ? "ring-[color:var(--color-matcha-400)]" : "bg-white ring-stone-200"}`}
                >
                  <span
                    className="pointer-events-none absolute inset-y-0 left-0 bg-[color:var(--color-matcha-50)] transition-[width] duration-200 ease-out"
                    style={{ width: progressPercent }}

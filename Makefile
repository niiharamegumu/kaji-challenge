...
SECURITY_BACKEND = cd backend && go run github.com/securego/gosec/v2/cmd/gosec@v2.22.8 -exclude-dir=internal/db/sqlc ./... && (go run golang.org/x/vuln/cmd/govulncheck@v1.1.4 ./... || true)
...
SECURITY_BACKEND = $(BACKEND_RUN) sh -c "cd /app/backend && go run github.com/securego/gosec/v2/cmd/gosec@v2.22.8 -exclude-dir=internal/db/sqlc ./... && (go run golang.org/x/vuln/cmd/govulncheck@v1.1.4 ./... || true)"

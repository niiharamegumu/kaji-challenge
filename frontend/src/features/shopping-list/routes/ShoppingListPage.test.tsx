...
describe("ShoppingListPage", () => {
  ...
  it("reorders items with move buttons", async () => {
    mockListShoppingItems.mockResolvedValue({
      data: {
        items: [
          {
    ...
    mockPostShoppingItemsReorder.mockResolvedValue({
      data: {
        items: [
          {
            id: "item-2",
    ...
    await user.click(
      await screen.findByRole("button", { name: "卵 を上に移動" }),
    );

    await waitFor(() => {
      expect(mockPostShoppingItemsReorder).toHaveBeenCalledWith({
